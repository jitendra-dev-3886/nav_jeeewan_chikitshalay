...
export interface Clinic { banners:{id:number;title:string;body:string;link_label:string;link_path:string}[]; redirects:{from_path:string;to_path:string}[]; profile:Profile; booking:Policy; hours:{id:number;weekday:number;start_time:string;end_time:string}[]; testimonials:{id:number;name:string;quote:string}[]; media:{id:number;path:string;alt:string;caption:string}[] }
...
export const useClinic=()=>useQuery({queryKey:['clinic'],queryFn:()=>api<Clinic>('/public/clinic')})
